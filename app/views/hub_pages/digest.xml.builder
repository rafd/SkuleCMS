     ...
     xml.item do      
       case feed_item.class.to_s
         ...
         when "LargePost" then
           ...
           xml.link           url_for :only_path => false, :controller=> feed_item.class.to_s.tableize, :action => 'show', :id => feed_item.id, :club_id => feed_item.club
           xml.description    feed_item.content
           xml.guid           url_for :only_path => false, :controller=> feed_item.class.to_s.tableize, :action => 'show', :id => feed_item.id, :club_id => feed_item.club
           xml.pubDate        feed_item.created_at
         when "Event" then
