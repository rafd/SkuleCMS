module EventCalendar
  ...
  module ClassMethods
    ...
    def events_for_date_range(start_d, end_d)
      self.find(
        :all,
        :conditions => [ '(? <= finish) AND (start <= ?)', start_d, end_d ],
        :order => 'start ASC'
      )
    end
    ...
    def create_event_strips(strip_start, strip_end, events)
      ...
      events.each do |event|
        cur_date = event.start.to_date
        end_date = event.finish.to_date
  ...
  module InstanceMethods
    ...
    def clip_range(start_d, end_d)
      ...
      if (start < start_d and finish >= start_d)
        clipped_start = start_d
      else
        clipped_start = start.to_date
      end
